...
let avoidRooms = ['W29S21', 'W29S23', 'W28S19', 'W29S19','W2419','W23S19','W22S19','W21S19','W32S23']      // 运行时参数，永不踏入这些房间
let avoidExits = {
    'E35N7': 'E35N6',
    'fromRoom': 'toRoom'
}   // 【未启用】运行时参数，单向屏蔽房间的一些出口，永不从fromRoom踏入toRoom
...
function generateCostMatrix(room, pos) {
    ...
    if (!room.controller || !room.controller.owner) {
        if (neutralCostMatrixClearDelay) {
            ...
            costMatrixCacheTimer[Game.time + neutralCostMatrixClearDelay].push({
                roomName: room.name,
                avoids: avoids
            });   // 记录创建时间
        }
...
function routeCallback(nextRoomName, fromRoomName) {    // 避开avoidRooms设置了的
    ...
    if (!(nextRoomName in costMatrixCache) && nextRoomName in Game.rooms) {   // 有视野没costMatrix
        let room = Game.rooms[nextRoomName];
        generateCostMatrix(room, room.find(room.findExitTo(fromRoomName))[0]);
    }
    return 1;
}
...
function findTemporalPath(creep, toPos, ops) {
    ...
    if (creep.pos.roomName != toPos.roomName) {
        checkTemporalAvoidExit(creep.pos, creep.room, bypassCostMat);
        ...
        route = route.reduce(routeReduce, { [creep.pos.roomName]: 1 });
...
function findPath(fromPos, toPos, ops) {
    ...
    if (fromPos.roomName != toPos.roomName) {
        ...
        route = route.reduce(routeReduce, { [fromPos.roomName]: 1 });
...
function betterMoveTo(firstArg, secondArg, opts) {
    ...
    found = findPathInCache(formalize(this.pos), formalize(toPos), creepCache, ops);
    if (found) {
        //this.say('cached');
...
module.exports = {
    ...
    addAvoidRooms: function (roomName) {
        let splited = reg1.parse(roomName);
        if (splited && splited.length == 5) {
    ...
    addAvoidExits: function (fromRoomName, toRoomName) {    // 【未启用】
        let splited1 = reg1.parse(fromRoomName);
        let splited2 = reg1.parse(toRoomName);
        if (splited1 && splited1.length == 5 && splited2 && splited2.length == 5) {
    ...
    print: function () {
        ...
        text += `\ncache search rate: ${((testCacheMiss + testCacheHits) / analyzeCPU.moveTo.calls).toFixed(4)}, total hit rate: ${(1 - missRate).toFixed(4)}`;
